...
export default function AdminReviews() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const res = await adminApi.get("/admin/reviews", { params: { page: String(page), limit: String(LIMIT) } }); setReviews(res.data.reviews); setTotal(res.data.total); }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Reviews</h1><p className="text-muted-foreground text-sm mt-1">{total} reviews</p></div>
      ...
      <div className="space-y-3">
        {loading ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-24 rounded-xl" />) :
          reviews.length === 0 ? <div className="text-center py-16 text-muted-foreground"><Star className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>No reviews yet.</p></div> :
          reviews.map((r) => (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1 flex-wrap">
                    ...
                    {r.verifiedPurchase && <span className="flex items-center gap-1 text-xs text-green-600"><CheckCircle className="w-3 h-3" />Verified</span>}
                  ...
                  {r.productId && <p className="text-xs text-muted-foreground mt-1">Product ID: {r.productId}</p>}
                </div>
                <Button size="sm" variant="ghost" onClick={() => setDeleteId(r.id)} className="h-8 px-2 text-destructive hover:bg-destructive/10 shrink-0"><Trash2 className="w-3.5 h-3.5" /></Button>
              </div>
            </CardContent></Card>
          ))
        }
