    ...
    <div className="min-h-screen bg-cream dark:bg-maroon-dark">
      ...
      <div className="container mx-auto px-4 py-20">
        ...
        <div className="max-w-3xl mx-auto text-center mb-20">
          ...
          <p className="text-base text-maroon-dark/70 dark:text-cream/70 leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {PILLARS.map((pillar) => (
            <div key={pillar.title} className="bg-white dark:bg-maroon/30 p-7 rounded-xl border border-gold/15 dark:border-maroon/50 text-center hover:border-gold dark:hover:border-gold/40 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group">
              <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-5 transition-transform duration-300 group-hover:scale-110" style={{ backgroundColor: `${pillar.color}15` }}>
                <i className={`fa-solid ${pillar.icon} text-2xl`} style={{ color: pillar.color }}></i>
              </div>
              <h3 className="font-serif text-xl text-maroon-dark dark:text-cream mb-2">{pillar.title}</h3>
              <p className="text-sm text-maroon-dark/65 dark:text-cream/65 leading-relaxed">{pillar.desc}</p>
            </div>
          ))}
        </div>

        {/* ── Image 5 (delivery/artisan) section ── */}
          ...
          <div className="relative z-10 px-8 md:px-14 py-12 md:py-16 max-w-xl">
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <div key={item.label} className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 px-3 py-1.5 rounded-full">
